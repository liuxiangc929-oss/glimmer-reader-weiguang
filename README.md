...
旧 `demo/` 目录暂时保留为历史 HTML Demo 归档参考，不再作为当前高保真实现的真相源。
...
3. 旧 `demo/` 仅作为历史 HTML Demo 归档参考，后续不再围绕它做高保真验收。
4. 完成 React 原型浏览器截图复核后，再把高保真网页用于作品集截图、面试演示和外部测试。
5. 继续验证已接入的 AI 今日总结和直接问，并在“基于原文回答”规范确认后再接入该节点的真实生成。
...
6. 用户主动结束阅读后，如果未满 5 分钟或本次阅读范围少于 2 页，App 只在阅读页内提示继续读一会儿再总结。
